...
VPN privada própria com exit node via VPS + painel web de administração + cliente desktop (Windows/Linux) em Go/Wails3/React. Arquitetura completa e decisões justificadas estão em [`PLAN.md`](./PLAN.md). Progresso e checklist estão em [`ROADMAP.md`](./ROADMAP.md) — **sempre atualize os checkboxes do `ROADMAP.md`** ao concluir uma tarefa nele listada. Trabalho da **branch/PR atual** vive em [`TASKS.md`](./TASKS.md) (skill `tasks`).

## Fatos que não mudam (não redescubra, não contradiga sem justificar)

- **VPS de produção real**: `206.189.224.72` (control-plane), Ubuntu 26.04 LTS, acesso root via chave SSH (sem senha configurada de propósito). Nó de dados: `66.29.147.100` (malha Compute, hostname `data` — Mongo/git/containers após enroll). Trate qualquer comando nesses hosts como produção — não é ambiente descartável.
- **Overlay WireGuard**: um `wg0`. **infra** `10.66.66.0/24` (hub `10.66.66.1`, malha/data, VIP `.254`). **users** `10.66.80.0/24` (devices). Custom no pool `10.66.80.0/20`. **Nunca** `10.10.0.0/16` ou `10.136.0.0/16`. Mongo/git só na infra. Ver [`docs/areas/networks.md`](./docs/areas/networks.md).
...
- `.cursor/skills/` — workflows executáveis para tarefas recorrentes:
  ...
  - Git/GitHub: criar branch (`start-task`), checklist da PR (`tasks` → `TASKS.md`), abrir PR (`ship-pr`), squash-merge (`land-pr`), releases pendentes (`release-status`) e publicação no catálogo (`marketplace-publish`) — ver [`PLAN.md` §13](./PLAN.md#13-versionamento-e-releases).
...
| Trabalho desta branch / PR? | [`TASKS.md`](./TASKS.md) (skill `tasks`) |
| Docs por área (compute, xgit, …) | [`docs/areas/`](./docs/areas/) |
