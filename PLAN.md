...
**XCODESPACES — canário Flask (Fase 57).** Repo playground **`teste`** no XGIT (owner **`rootkit`**) inclui `web/flask/app.py` + `scripts/demo-flask.sh` escutando **`0.0.0.0:8080`** para smoke-test da Fase 56 (`http://demo-cs-<id>.corp.ihuull.com:8080/` na VPN). Imagem bakeia `python3-flask`. O agente espelha `$ argv` + stdout no terminal **XCODESPACES** após `run_terminal` (sem prefixo `# agent:` — comentário de shell). §3.6.
...
| **57. Canário Flask** | repo `teste` (rootkit) + Flask `0.0.0.0:8080` + espelho terminal agente | smoke-test da 56; sem A público |
