...
| API XVPN — listener do túnel (Fase 14) | `10.66.66.1:8080` | **Mesma porta, outra interface.** Bind só em `wg0`, nunca em `0.0.0.0`/`eth0`; sem entrada no Nginx e sem domínio. Firewall: `ufw allow in on wg0 to any port 8080 proto tcp`. Existe porque o HTTPS do painel **não** passa dentro do túnel (a rota `/32` de exceção do cliente o desvia — ver §6.9), então é o único caminho pelo qual um peer chega à API com um `10.66.66.x` como IP de origem. Só as rotas `GET /api/me` e `POST /api/me/ssh-key` respondem aqui, atrás de um middleware que exige `RemoteIP()` na sub-rede da VPN |
...
**Configuração global do Samba (`map to guest`):** o `smb.conf` global tem `map to guest = Bad User` (não `never`) — sem isso, o `guest ok = yes` dos shares per-user é rejeitado e o caminho "VPN como auth" não funciona. **Isso não enfraquece o share `[shared]`** (que tem `guest ok = no` + `valid users = @xvpn-samba`): `map to guest` só mapeia pra guest tentativas em shares que *aceitam* guest, e `[shared]` explicitamente não aceita.
