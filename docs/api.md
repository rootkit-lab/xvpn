...
`visibility` (quem, ACL) ≠ `network` (onde). App `network: vpn` não lista nem baixa em `marketplace.ihuull.com` sem túnel; aparece em `*.corp` ou quando o peer está na VPN (`10.66.66.0/24`). `network: public` aparece na loja pública com JWE (nunca anônimo).
