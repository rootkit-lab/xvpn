...
- Chat nativo **CHAT / COPILOT EDITS** do OpenVSCode **não** é o produto (Fase 52). Machine settings escondem o command center e a secondary sidebar; a extensão desinstala Copilot/Continue/Cline se o usuário instalar e abre o agente no **activity bar**. Skills = `.cursor/skills`; agente = `AGENTS.md`; tools só no clone (write/term com **Aplicar**). Sem Copilot/Continue bakeados; sem `docker.sock`. Troca da extensão = rebuild da imagem + **Recreate** (start antigo não troca a layer). Troca do parse LLM / `tool_calls` = só deploy do `xvpn-server`.
