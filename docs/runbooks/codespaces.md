...
Na VPN: `http://demo-<nome>.corp.ihuull.com:<porta>` chega no processo **dentro do container** (VIP `10.66.66.254` → DNAT). **Não** uses `http://cs-<id>.corp:porta` — o IDE nesse host manda HSTS e o Chrome sobe para HTTPS (landing/`xvpn-server` em `.1:8080`). O A do demo é `/etc/xvpn/demo.hosts` (`addn-hosts=`). `systemctl reload` (SIGHUP) relê esse ficheiro; `host-record=` em `00-xvpn-demo.conf` **não** entra no processo até um `restart`. Nome no painel XCODESPACES → **Demo**. O app tem que escutar `0.0.0.0` (Vite: `--host`). Sem A público. Deploy do helper `xvpn-user-provision` **e** do `xvpn-server`.
...
- Chat nativo **CHAT / COPILOT EDITS** do OpenVSCode **não** é o produto (Fase 52). Machine settings escondem o command center e deixam a **secondary sidebar visível**. A view do agente entra em `viewsContainers.secondarySideBar` (direita). O 1.98 só vinha com `activitybar`/`panel` — a imagem aplica `patch-auxiliary-bar.js`. `workbench.panel.chat` com AI off caía no Explorer (esquerda). A extensão desinstala Copilot/Continue/Cline se o usuário instalar, fecha o Chat/Edits nativos e foca a auxiliary bar. Chrome: modos **Agent / Ask / Debug / Plan** + seletor de modelo (`GET /api/xcodespaces/llm/models`; override no `POST /chat` só se o ID estiver no catálogo do provedor). Timeline: Thinking, resumo expansível e cards de tool (sem `tool N…`). Composer `@` `#` `$` `/`; terminal espera o comando (`python3` + campo `env`). MCP think/memory/docs. Logs em `.cursor/agent/` (fallback `/tmp/xcs-agent`); Review/Stop. Workspace = clone `xgit.corp`, não GitHub/fork. `.cursor/hooks.json` só inspecionado (não executa o `command`). O GET também devolve `git_name`/`git_email`; a extensão (e o helper no Create/Start) grava `user.name`/`user.email` no clone — sem isso o Source Control mostra *configure your user.name*. Skills = `.cursor/skills`; agente = `AGENTS.md` (ou contrato ihuull se o clone não tiver); tools só no clone, com `glob` (write com **Aplicar** / **Sempre**; terminal ainda confirma). Loop no container: teto **24** turns; no teto o agente resume o que já leu (Ask, sem tools) em vez de só pedir para reformular. Sem Copilot/Continue bakeados; sem `docker.sock`. Troca da extensão = rebuild da imagem + **Recreate** (start antigo não troca a layer). Troca do parse LLM / `tool_calls` / `/models` / `maxLLMChatMsgs` = só deploy do `xvpn-server`. Identidade no Create/Start = helper `xvpn-user-provision`.
