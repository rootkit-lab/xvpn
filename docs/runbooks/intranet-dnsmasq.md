...
5. **Aplicar no dnsmasq** — grava `/etc/dnsmasq.d/xvpn-corp.conf` e `/etc/xvpn/dnsmasq-records.hosts` (nunca `.hosts` dentro de `/etc/dnsmasq.d/` — o start do Ubuntu lê tudo como config). O A do demo (`demo-*.corp`) fica em `/etc/xvpn/demo.hosts` (`addn-hosts=`), não em `host-record=` — SIGHUP relê hosts, não os `.conf`.
