...
var workflowTemplates = []WorkflowTemplate{
	...
	},
	{
		...
		Description: "npm publish no registry xgit.corp. Token só no env do runner.",
		Languages:   []string{"JavaScript"}, Icon: "package",
		Script: "#!/bin/sh\nset -eu\n: \"${XVPN_PACKAGES_TOKEN:?XVPN_PACKAGES_TOKEN ausente no runner}\"\nREGISTRY=\"https://xgit.corp.ihuull.com/api/packages/{{REPO}}/npm/\"\nprintf '//xgit.corp.ihuull.com/api/packages/{{REPO}}/npm/:_authToken=%s\\n' \"$XVPN_PACKAGES_TOKEN\" > .npmrc.publish\nnpm publish --registry \"$REGISTRY\" --userconfig .npmrc.publish\nrm -f .npmrc.publish\n",
	},
	{
		ID: "pypi-xgit", Name: "Publish PyPI to XGIT", Category: "publish",
		Description: "twine contra a Simple API do XGIT. Token só no env do runner.",
		Languages:   []string{"Python"}, Icon: "package",
		Script: "#!/bin/sh\nset -eu\n: \"${XVPN_PACKAGES_TOKEN:?XVPN_PACKAGES_TOKEN ausente no runner}\"\nTWINE_USERNAME=xgit TWINE_PASSWORD=\"$XVPN_PACKAGES_TOKEN\" twine upload --non-interactive --repository-url https://xgit.corp.ihuull.com/api/packages/{{REPO}}/pypi dist/*\n",
	},
	{
	...
	},
	{
		...
		Description: "mvn deploy no registry Maven do XGIT. Token só no env do runner.",
		Languages:   []string{"Java"}, Icon: "package",
		Script: "#!/bin/sh\nset -eu\n: \"${XVPN_PACKAGES_TOKEN:?XVPN_PACKAGES_TOKEN ausente no runner}\"\nSETTINGS=$(mktemp)\ncat > \"$SETTINGS\" <<'EOF'\n<settings>\n  <servers>\n    <server>\n      <id>xgit</id>\n      <username>xgit</username>\n      <password>${env.XVPN_PACKAGES_TOKEN}</password>\n    </server>\n  </servers>\n</settings>\nEOF\nmvn --batch-mode -s \"$SETTINGS\" deploy -DaltDeploymentRepository=xgit::default::https://xgit.corp.ihuull.com/api/packages/{{REPO}}/maven\nrm -f \"$SETTINGS\"\n",
	},
	{
		ID: "nuget-xgit", Name: "Publish NuGet to XGIT", Category: "publish",
		Description: "dotnet nuget push no feed do XGIT. Token só no env do runner.",
		Languages:   []string{"C#"}, Icon: "package",
		Script: "#!/bin/sh\nset -eu\n: \"${XVPN_PACKAGES_TOKEN:?XVPN_PACKAGES_TOKEN ausente no runner}\"\ndotnet nuget push *.nupkg --source https://xgit.corp.ihuull.com/api/packages/{{REPO}}/nuget/index.json --api-key \"$XVPN_PACKAGES_TOKEN\"\n",
	},
	{
		ID: "gem-xgit", Name: "Publish RubyGems to XGIT", Category: "publish",
		Description: "gem push no host XGIT. Token só no env do runner.",
		Languages:   []string{"Ruby"}, Icon: "package",
		Script: "#!/bin/sh\nset -eu\n: \"${XVPN_PACKAGES_TOKEN:?XVPN_PACKAGES_TOKEN ausente no runner}\"\nGEM_HOST_API_KEY=\"$XVPN_PACKAGES_TOKEN\" gem push --host https://xgit.corp.ihuull.com/api/packages/{{REPO}}/rubygems *.gem\n",
	},
}
