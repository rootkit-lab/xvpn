...
// usado atrás de heros/telas de destaque (landing, dashboard) para reforçar
// a identidade "rede privada" sem pesar no bundle: tudo é SVG + CSS,
// nenhuma lib de 3D/mapa. Puramente decorativo — `aria-hidden` e
// `pointer-events-none`, nunca compete com conteúdo real por atenção.
const NODES = [
  { x: 100, y: 60 },
  { x: 220, y: 90 },
  { x: 60, y: 160 },
    ...
    <div className={`pointer-events-none select-none ${className}`} aria-hidden="true">
      <svg viewBox="0 0 380 260" className="h-full w-full overflow-visible text-primary">
        <g className="animate-drift" style={{ transformOrigin: "190px 130px" }}>
          {/* Globo wireframe: um contorno + elipses simulando meridianos/paralelos */}
          ...
          {/* Arcos de conexão entre "nós" — dash animado dá sensação de dado viajando */}
          {ARCS.map(([from, to], i) => (
            <path
              key={i}
          ...
          {/* Nós: ponto sólido + halo pulsando, defasado por índice */}
          {NODES.map((n, i) => (
            <g key={i}>
              <circle
                cx={n.x}
